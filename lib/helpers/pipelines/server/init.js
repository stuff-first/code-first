    ...
    return shell.exec(initServerCmd).then(() => {
        ...
        results.push(shell.exec(installCmd).then(() => {
            return fs.updateFile('./server/package.json', data => {
                ...
                pkgObj.scripts.cpConfigJson = "mkdir dist dist/config; cp src/config/config.json dist/config/config.json";
                pkgObj.scripts.build = "babel src --out-dir dist; npm run cpConfigJson";
